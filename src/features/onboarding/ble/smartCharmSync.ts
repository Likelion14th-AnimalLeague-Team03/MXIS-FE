...
export async function collectSmartCharm(connection: SmartCharmConnection) {
  ...
  await charmOutbox.remember(ownerId, serialNumber, device.id);
...
export async function uploadAndAcknowledgeSmartCharm(ownerId: string, serial: string, backendId: number) {
  ...
  try {
    ...
    const connection = getSmartCharmConnection(ownerId, serial) ?? await connectSmartCharm(box.bleDeviceId, ownerId, { expectedSerial: serial });
