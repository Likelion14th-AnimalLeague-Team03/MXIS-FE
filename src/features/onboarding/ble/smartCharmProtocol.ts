...
export function encodeCommand(command: string) {
  ...
  if (bytes.length > 20) throw new Error("UART 명령이 단일 Write 크기를 초과했습니다.");
...
export function parseSensorReadingLine(line: string): SensorReadingDto {
  ...
  if (crc16CcittFalse(bytes) !== parseInt(f[7], 16)) throw new Error("센서 CRC 검증에 실패했습니다.");
...
export function sameReading(a: SensorReadingDto, b: SensorReadingDto) {
  return a.sequence === b.sequence && a.measuredAt === b.measuredAt &&
