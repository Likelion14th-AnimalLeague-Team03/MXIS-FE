import {
  base64ToBytes, encodeCommand, parseInteger, parseSensorReadingLine, parseStatus,
...
export function createUartSession(
  ...
  timings = { command: 5000, sync: 15000, progress: 3000, busy: 300 },
) {
    ...
    const response = new Promise<string>((resolve, reject) => {
      ...
      pending = {
        ...
        accept: (line) => {
          ...
          if (progressTimeout && /^(SYNC_BEGIN,|R,|SYNC_END,)/.test(line)) resetProgress();
          return done;
        },
      };
    });
  ...
  async function command(value: string, accept: (line: string) => boolean) {
    for (let attempt = 0; attempt < 4; attempt++) {
      const line = await request(value, accept);
  ...
  return {
    ...
    verifyProfile: () => enqueue(async () => {
      const line = await command("PROFILE", (value) => value.startsWith("PROFILE,"));
    ...
    acknowledge: (sequence: number) => enqueue(async () => {
      ...
      const line = await command(`ACK ${sequence}`, (value) => value.startsWith("ACK,"));
      const f = line.split(",");
