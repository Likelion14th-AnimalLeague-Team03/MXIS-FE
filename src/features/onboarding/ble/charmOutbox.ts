...
export class CharmOutbox {
  ...
  remember(owner: string, serial: string, bleDeviceId: string, allowedServiceUuids?: string[]) {
    return this.update(owner, serial, (box) => {
      box.bleDeviceId = bleDeviceId;
      if (allowedServiceUuids) box.allowedServiceUuids = allowedServiceUuids;
    });
  }
