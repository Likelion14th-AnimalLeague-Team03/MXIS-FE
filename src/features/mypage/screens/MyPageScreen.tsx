...
const OTHER_LINKS = ["서비스 이용약관", "개인정보 수집·이용 약관", "브랜드 소식 및 마케팅 알림 약관"];

// 목업 값: 소셜 로그인 계정이면 비밀번호 변경 칸을 숨겨요.
// 실제 로그인 연동 붙으면 이 값을 계정 정보에서 가져오면 돼요.
  ...
  return (
    <SafeAreaView edges={["top"]} className="flex-1 bg-concierge-bg">
      <ScrollView className="flex-1 px-6" contentContainerClassName="pb-8">
        <Text className="mt-10 text-xl font-bold text-concierge-text">마이페이지</Text>
        ...
        <View className="mt-[20px] px-3">
          <Pressable onPress={toggleInfo} className="flex-row items-center justify-between py-2">
            <Text className="text-sm text-concierge-textSecondary">내 정보 변경</Text>
            <View style={{ transform: [{ rotate: infoExpanded ? "-90deg" : "90deg" }] }}>
          ...
          {infoExpanded ? (
            <View className="mb-3 mt-1 gap-px overflow-hidden rounded-xl bg-concierge-chip">
              <View className="flex-row items-center justify-between px-4 py-3">
                <Text className="text-sm text-concierge-textSecondary">이름</Text>
                <TextInput
                  value={name}
              ...
              <View className="flex-row items-center justify-between px-4 py-3">
                <Text className="text-sm text-concierge-textSecondary">이메일</Text>
                <TextInput
                  value={email}
              ...
              {!IS_SOCIAL_LOGIN ? (
                <View className="flex-row items-center justify-between px-4 py-3">
                  <Text className="text-sm text-concierge-textSecondary">비밀번호</Text>
                  <TextInput
                    value={password}
          ...
          {infoExpanded ? (
            <PrimaryButton label="저장하기" onPress={handleSaveInfo} className="mb-3" />
          ) : null}
      ...
      >
        <View className="flex-1 items-center justify-center bg-black/40 px-6">
          <View className="w-full rounded-2xl bg-concierge-surfaceMuted p-5">
            <Text className="text-lg font-bold text-concierge-text">로그아웃하시겠어요?</Text>
            <Text className="mt-2 text-sm text-concierge-textSecondary">
              MXIS Charm은 계속 기록하지만 다시 로그인하기 전까지 데이터가 동기화되지 않을 수
              있습니다.
            </Text>
            <PrimaryButton
              label="로그아웃"
