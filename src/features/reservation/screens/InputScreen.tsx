...
export function InputScreen() {
  ...
  return (
    <SafeAreaView edges={["top"]} className="flex-1 bg-concierge-bg">
      <Text className="px-6 pt-4 text-xl font-bold text-concierge-text">케어 컨시어지 예약</Text>

      <ScrollView className="flex-1 px-6" contentContainerClassName="pb-4" keyboardShouldPersistTaps="handled">
        <Card className="mt-4 flex-row items-center gap-3 border-0 bg-concierge-surfaceMuted px-4 py-4">
          <Image source={productPhoto} className="size-[78px] rounded-2xl" resizeMode="cover" />
          <View>
            <Text className="text-base font-semibold text-[#121212]">MCM Aren Shopper</Text>
            <Text className="mt-1 text-[13px] text-[#63635E]">Visetos Canvas · Cognac</Text>
            <Text className="mt-1 text-[13px] font-medium text-[#121212]">함께한 외출 50회</Text>
          </View>
        </Card>
        ...
        <Card className="mt-4">
          ...
          <SelectRow
            label="방문 일정"
            value={draft.date && draft.time ? `${formatDateShort(draft.date)} · ${draft.time}` : null}
        ...
        <Card className="mt-4 px-4 py-4">
          <Text className="text-base font-bold text-concierge-text">요청사항</Text>
        ...
        <View className="mt-5 flex-row items-center gap-1.5">
          <WarningIcon />
          <Text className="text-sm font-bold text-concierge-text">예약 안내</Text>
        </View>
      ...
      <View className="px-6 pb-4 pt-2">
        <PrimaryButton label="예약 요청하기" onPress={handleSubmit} disabled={!canSubmit} />
      </View>
    </SafeAreaView>
  );
}
