...
const reading = (sequence, measuredAt = 1800000000) => ({
  sequence, measuredAt, temperature: 24.7, humidity: 45, maxShock: 0.09, motionCount: 0,
  raw: { temperatureX100: 2470, humidityX100: 4500, maxShockX100: 9 },
});
...
test("CRC matches standard vector and real firmware reading #150", () => {
  ...
  assert.deepEqual(p.parseSensorReadingLine("R,150,0,2480,4500,10,0,1FFE"), {
    ...reading(150, 0), temperature: 24.8, maxShock: 0.1,
    raw: { temperatureX100: 2480, humidityX100: 4500, maxShockX100: 10 },
  });
});
test("signed temperature and unsigned boundaries survive CRC parsing", () => {
  const r = {
    sequence: 0xffffffff, measuredAt: 0xffffffff, temperature: -327.68,
    humidity: 655.35, maxShock: 655.35, motionCount: 65535,
    raw: { temperatureX100: -32768, humidityX100: 65535, maxShockX100: 65535 },
  };
...
test("reject damaged CRC, extra fields, decimals, invalid numbers", () => {
  ...
  for (const bad of [valid.replace("2480", "2481"), valid + ",", valid.replace("150", "1.5"), valid.replace("150", "12x"), valid.replace("150", "0"), valid.replace("4500", "-1")]) {
...
test("DIAG, BATTERY and STOP responses are parsed exactly", async (t) => {
  ...
  assert.deepEqual(await h.session.getDiagnostics(), {
    bleRxBytes: 10, commandErrors: 2, sensorErrors: 1, missedImuSamples: 3,
  });
...
test("collision is sticky, wrong backend and device reset block ACK", async () => {
  ...
  await assert.rejects(outbox.saveReadings(owner, serial, [{
    ...reading(1), temperature: 30,
    raw: { ...reading(1).raw, temperatureX100: 3000 },
  }]));
...
Module._load = function (request, parent, ...rest) {
  if (request === "@/shared/api/client" && path.basename(parent?.filename ?? "") === "onboardingApi.ts") return { apiClient: { post: (...args) => httpPost(...args) } };
...
test("native connection policy requires NUS and rejects the old UUID", () => {
...
test("both modes scan without an advertising filter", () => {
  const allowed = [p.SMART_CHARM_NUS_SERVICE_UUID];
  assert.equal(native.getCharmScanServiceUuids("nearby", allowed), null);
  assert.equal(native.getCharmScanServiceUuids("service", allowed), null);
...
test("service mode shows a Smart Charm name even when its UUID is not advertised", () => {
  const allowed = [p.SMART_CHARM_NUS_SERVICE_UUID];
  for (const name of ["SmartCharmOBLE", "MXIS Charm", "SC-OB-000001"]) {
    assert(native.isVisibleCharmScanCandidate({ name, serviceUUIDs: null }, "service", allowed));
  }
});
...
test("native discovery uses the measured UART service and characteristic properties", async (t) => {
  ...
  let removed = false;
  const device = {
    services: async () => [{ uuid: serviceUUID.toLowerCase() }],
    characteristicsForService: async () => [
      { uuid: notifyUUID, isNotifiable: true },
      { uuid: writeUUID, isWritableWithoutResponse: true },
    ],
    monitorCharacteristicForService: (service, characteristic) => {
      assert.equal(service, serviceUUID.toLowerCase());
      assert.equal(characteristic, notifyUUID);
      return { remove() { removed = true; } };
    },
    onDisconnected: () => ({ remove() {} }),
    writeCharacteristicWithoutResponseForService: async () => {},
  };
  const session = await native.createSmartCharmUartSession(device, [serviceUUID]);
  t.after(() => session.dispose());
  session.dispose();
  assert(removed);
});
...
test("HTTP adapter keeps backend DTO mapping and uploads all valid records", async (t) => {
  ...
  httpPost = async (url, body, options) => {
    ...
    assert(!("isOuting" in body.readings[0]));
